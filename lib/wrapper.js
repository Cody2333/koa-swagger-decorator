...
const validateMiddleware = (parameters) => async (ctx, next) => {
  ...
  if (parameters.body instanceof Array && parameters.body.length > 0) {
    const bodyExpect = Object.keys(parameters.body[0].schema.properties).map(key => Object.assign({ name: key }, parameters.body[0].schema.properties[key]));
    ctx.request.body = validate(ctx.request.body, bodyExpect);
  }
  await next();
};
